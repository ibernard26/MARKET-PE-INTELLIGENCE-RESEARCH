...
def test_observation_is_append_only():
    c = mem()
    o = Observation("D1", "2026-01-05T00:00:00", "sec_filing", target_price=95.0)
...
def test_point_in_time_read_excludes_the_future():
    c = mem()
    record_observation(Observation("D1", "2026-01-05T00:00:00", "s", target_price=95.0), conn=c)
    record_observation(Observation("D1", "2026-02-01T00:00:00", "s", target_price=98.0), conn=c)
...
def test_missing_fields_stay_none_not_fabricated():
    c = mem()
    record_observation(Observation("D1", "2026-01-05T00:00:00", "s", offer_price=100.0), conn=c)
...
def test_provenance_is_required():
    with pytest.raises(ValueError):
        Observation("D1", "2026-01-05T00:00:00", "")   # empty source rejected


def test_invalid_status_rejected():
    with pytest.raises(ValueError):
        Observation("D1", "2026-01-05T00:00:00", "s", status="maybe")


def test_json_attrs_roundtrip():
    c = mem()
    record_observation(Observation("D1", "2026-01-05T00:00:00", "s",
...
def test_event_type_must_be_known():
    c = mem()
    with pytest.raises(ValueError):
        ev.record_event("D1", "2026-01-05", "made_up_event", "s", conn=c)
...
def test_resolution_cannot_precede_announcement():
    ...
    with pytest.raises(ev.LifecycleError):
        ev.record_event("D1", "2026-01-01", "closing", "s", conn=c)   # before announce
...
def test_events_as_of_is_point_in_time():
    c = mem()
    ev.record_event("D1", "2026-02-01", "announcement", "s", conn=c)
    ev.record_event("D1", "2026-05-01", "closing", "s", conn=c)
